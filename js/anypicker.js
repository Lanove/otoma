  ...
  AnyPicker.prototype = {
    ...
    init: function () {
      ...
      if ($.CF.isValid(apo.elem)) {
        ...
        if (apo.tmp.sElemTag === "input" || apo.tmp.sElemTag === "button") apo.tmp.oElemValid.bIsInput = true;
        else if (
          apo.tmp.sElemTag === "ul" ||
          apo.tmp.sElemTag === "ol" ||
          apo.tmp.sElemTag === "dl"
      ...
      if ($.CF.isValid(apo.setting.inputElement)) {
        apo.tmp.sInputElemTag = $(apo.setting.inputElement)
          .prop("tagName")
          .toLowerCase();
        if (apo.tmp.sInputElemTag === "input" || apo.tmp.sInputElemTag === "button")
    ...
    _adjustPicker: function (iNumOfComp) {
      ...
      if ($.CF.compareStrings(apo.setting.theme, "Windows")) {
        ...
        iContTop += 2;
        iContentTop =
          $(apo.tmp.overlaySelector).find(".ap-header").height() - iContTop;
        $(apo.tmp.overlaySelector)
          .find(".ap-content-top")
          .css({ top: iContentTop });
        iContTop -= 2;
        iContentBottom =
          $(apo.tmp.overlaySelector).find(".ap-footer").height() + iContTop;
        $(apo.tmp.overlaySelector)
  ...
  AnyPicker.prototype = $.extend(AnyPicker.prototype, {
    ...
    _setScrollingData: function (e) {
      ...
      apo.tmp.iMaxTopPos =
        ...
        $(apo.tmp.oScrollingComp).parent().innerHeight();
	...
	*/
    ...
    _getOffset: function (oElem, bAbs) {
      var sMatrix = $(oElem).hasClass("ap-component-data")
          ? $(oElem).css("transform")
          : $(oElem).find(".ap-component-data").css("transform"),
        iOffset = 0;
  ...
  AnyPicker.prototype = $.extend(AnyPicker.prototype, {
	...
	*/
    ...
    parseDisableValues: function () {
      ...
      if (
        $.CF.isValid(apo.tmp.oMinMax.min) &&
        $.CF.isValid(apo.tmp.oMinMax.max)
      ) {
