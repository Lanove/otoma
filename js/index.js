      ...
      if (id == "contactUs") {
        ...
        $("#js-reset-filter").click(function () {
          $("#js-provinsi-selector")
            ...
            .text("Pilih Provinsi");
          $(
            "#js-provinsi-selector>.city-picker__selector__items>div.city-picker__selector--same-as-selected"
          ).removeClass("city-picker__selector--same-as-selected");
          $("#js-kota-selector").remove();
          $("#js-kota-label").remove();
          $("#pembeli").children(".product-box, .product-not-found").remove();
          requestAJAX(
            "GlobalService",
            {
...
$(document).ready(function () {
  ...
  $(document).on("click", ".product-box, .delete-icon", function (e) {
    e.stopPropagation();
    let id;
    if ($(this).hasClass("product-box")) {
      id = this.id;
      $(".absolute-overlay").removeClass("loaded");
      $("#content").html("");
      $("#content").load(
            ...
            requestAJAX(
              ...
              function (response) {
                ...
                if (response.status) {
                  ...
                  $("#js-gambar-1").attr(
                    "src",
                    response.product.gambar1.replace("../", "")
                  );
                  $("#js-gambar-2").attr(
                    "src",
                    response.product.gambar2.replace("../", "")
                  );
                  $("#js-gambar-3").attr(
                    "src",
                    response.product.gambar3.replace("../", "")
                  );
                  $("#js-price").text(
                    formatRupiah(response.product.hargaBarang)
                  );
                  $("#js-title").text(response.product.namaBarang);
                  $("#js-rating").text(
                    parseFloat(response.product.rating).toFixed(1)
                  );
                  $("#js-review").text(response.product.review + " Ulasan");
                  $("#js-photo").attr(
                    "src",
                    response.product.photoPath.replace("../", "")
                  );
                  $("#js-nama").text(response.product.nama);
                  $("#js-from").text(
                    response.product.provinsi + ", " + response.product.kota
                  );
      ...
      bootbox.confirm({
        ...
        message: `Apakah anda yakin ingin menghapus produk dengan nama <b>${$(
          this
        )
          .parent()
          .find(".product-box__title-label>span")
          .text()}</b>?`,
        buttons: {
          cancel: {
            label: '<i class="fa fa-times"></i> Tidak',
